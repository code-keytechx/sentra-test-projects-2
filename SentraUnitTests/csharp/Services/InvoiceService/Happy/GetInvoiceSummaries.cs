...
public class InvoiceServiceTests
{
    ...
    public void GetInvoiceSummaries_ReturnsPagedResults_WithValidInputs()
    {
        ...
        _invoiceRepositoryMock.Setup(repo => repo.AsQueryable()).Returns(invoices.AsQueryable());
        ...
        Assert.Equal("Customer2", result.Items[1].CustomerName);
    }
    ...
    public void GetInvoiceSummaries_ReturnsFilteredResults_WithSearchTerm()
    {
        ...
        {
            new InvoiceSummary { Id = 1, CustomerName = "Customer1", InvoiceDate = DateTime.Now, TotalAmount = 100, Status = "Paid" }
        };

        _invoiceRepositoryMock.Setup(repo => repo.AsQueryable()).Returns(invoices.AsQueryable());
        _mapperMock.Setup(mapper => mapper.Map<List<InvoiceSummary>>(new List<Invoice> { invoices.First() })).Returns(invoiceSummaries);
    ...
    public void GetInvoiceSummaries_SortsByInvoiceDateDescending()
    {
        ...
        var invoices = new List<Invoice>
        {
            new Invoice { Id = 1, CustomerName = "Customer1", InvoiceDate = DateTime.Now.AddDays(-1), TotalAmount = 100, Status = "Paid" },
